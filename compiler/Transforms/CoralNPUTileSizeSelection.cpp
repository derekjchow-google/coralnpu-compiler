  ...
  for (unsigned i = 0; i < map.getNumResults(); ++i) {
    ...
    if (lb && ub) {
      ...
      linalgOp.emitWarning()
          << "could not estimate bound for dimension " << i << " of operand #"
          << operand->getOperandNumber() << " (expression: " << os.str()
          << ") of '" << linalgOp->getName().getStringRef()
...
static int64_t estimatePackOperandFootprint(linalg::PackOp packOp,
                                            OpOperand *operand,
                                            ArrayRef<int64_t> tileSizes) {
  ...
  int64_t numElements = 1;
  int64_t elementBytes = type.getElementType().getIntOrFloatBitWidth() / 8;
  int64_t rank = type.getRank();
  auto shape = type.getShape();
  int64_t numLoops = tileSizes.size();

  if (operand->get() == packOp.getSource()) {
    // Input operand (rank N)
    auto perm = packOp.getOuterDimsPerm();
    auto innerDimsPos = packOp.getInnerDimsPos();
    auto mixedInnerTiles = packOp.getMixedTiles();

    for (int64_t l = 0; l < numLoops; ++l) {
      int64_t inputDim = perm.empty() ? l : perm[l];
      int64_t tileSize = tileSizes[l];
      if (tileSize == 0) {
        numElements *= shape[inputDim];
      } else {
        int64_t innerTileSize = 1;
        auto it = llvm::find(innerDimsPos, inputDim);
        if (it != innerDimsPos.end()) {
          size_t k = std::distance(innerDimsPos.begin(), it);
          if (auto optConst = getConstantIntValue(mixedInnerTiles[k])) {
            innerTileSize = *optConst;
          }
        }
        numElements *= std::min(shape[inputDim], tileSize * innerTileSize);
      }
    }
  } else if (operand->get() == packOp.getDest()) {
    // Output operand (rank N + M)
    // 1. Outer dims (tiled)
    for (int64_t l = 0; l < numLoops; ++l) {
      int64_t tileSize = tileSizes[l];
      if (tileSize == 0) {
        ...
        numElements *= std::min(shape[l], tileSize);
      }
    }
    // 2. Inner dims (full)
...
static int64_t estimateUnpackOperandFootprint(linalg::UnPackOp unpackOp,
                                              OpOperand *operand,
                                              ArrayRef<int64_t> tileSizes) {
  Value value = operand->get();
  auto type = dyn_cast<RankedTensorType>(value.getType());
  ...
  if (operand->get() == unpackOp.getDest()) {
    // Output operand (rank N)
    for (int64_t i = 0; i < rank; ++i) {
      int64_t tileSize = tileSizes[i];
      if (tileSize == 0) {
        numElements *= shape[i];
      } else {
        numElements *= std::min(shape[i], tileSize);
      }
    }
  } else if (operand->get() == unpackOp.getSource()) {
    // Input operand (rank N + M)
    auto perm = unpackOp.getOuterDimsPerm();
    auto innerDimsPos = unpackOp.getInnerDimsPos();
    auto mixedInnerTiles = unpackOp.getMixedTiles();

    // 1. Outer dims (tiled)
    for (int64_t l = 0; l < numLoops; ++l) {
      int64_t srcDim = perm.empty() ? l : perm[l];
      int64_t tileSize = tileSizes[l];
      if (tileSize == 0) {
        numElements *= shape[srcDim];
      } else {
        int64_t innerTileSize = 1;
        auto it = llvm::find(innerDimsPos, srcDim);
        if (it != innerDimsPos.end()) {
          size_t k = std::distance(innerDimsPos.begin(), it);
          if (auto optConst = getConstantIntValue(mixedInnerTiles[k])) {
            innerTileSize = *optConst;
        ...
        int64_t neededTiles = (tileSize + innerTileSize - 1) / innerTileSize;
